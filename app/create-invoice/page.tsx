          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left mb-8">
              ...
              <tbody>
                ...
                return item.type === "subtotal"?<tr key={index} className="border-b transition-colors"
                style={{color:"black"}}
                >
                     <td colSpan={3} className="py-4 pe-5 font-semibold ps-3 text-right"
                    >Sub Total</td>
                    <td className="py-4 font-bold ps-3"
                    >₦{invoiceData.items.filter((d,i)=>i <= index).map((a)=>a.unitPrice * a.quantity).reduce((a,b)=>a + b,0)}</td>
                  </tr>:item.type === "section"?<tr key={index} className="border-b transition-colors"
